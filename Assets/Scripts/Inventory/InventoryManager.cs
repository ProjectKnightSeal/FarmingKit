...
public class InventoryManager : SingletonMonobehaviour<InventoryManager>, ISaveable
{
    ...
    private int[] selectedInventoryItem;//the index of the array is the inventoyr list, and the value is the item code
    ...
    public void AddItem(InventoryLocation inventoryLocation,Item item)
    {
        ...
        EventHandler.CallInventoryUpdatedEvent
            (inventoryLocation, inventoryLists[(int)inventoryLocation]);
    }
    ...
    {
        //if fromItem index and toItemIndex within the bounds of the list, not the same, and greater than or equal to zero
    ...
    //Returns the itemDetails (from the SO_ItemList) for the currently selected in the inventoryLocation , or null if an item isn't selected
        ...
        if(gameSave.gameObjectData.TryGetValue(ISaveableUniqueID,out GameObjectSave gameObjectSave))
        {
            ...
            if(gameObjectSave.sceneData.TryGetValue(Settings.PersistentScene,out SceneSave sceneSave))
            {
                ...
                if (sceneSave.listInvItemArray != null)
                {
                    ...
                    //Send events that inventory has been updatad
                    for(int i = 0; i < (int)InventoryLocation.count; i++)
                    {
    ...
    //    {
    //        Debug.Log("Item Description:" + InventoryManager.Instance.
    //            GetItemDetails(inventoryItem.itemCode).itemDescription +
